...
function setupFindingSuccessVideos() {
  ...
  for (let i=0; i<urls.length; i++) {
    ...
    findingSuccessVideoBoxes.push({ x, y, w:boxWidth, h:boxHeight });
...
function drawFindingSuccessScene() {
  background(30);
  textFont(titleFont); textSize(48); fill(255);
  textAlign(CENTER, TOP);
  text("Finding Success Era", width/2, 50);

  for (let i=0; i<findingSuccessVideoBoxes.length; i++) {
    let b = findingSuccessVideoBoxes[i];
    fill(80); rect(b.x,b.y,b.w,b.h,20);

    if (isCharacterTouchingBox(b)) findingSuccessVideos[i].show();
    else                           findingSuccessVideos[i].hide();
  }
